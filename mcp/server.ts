...
import { getKpi, listArchivedKpis, listKpis, parseCriteria } from "../src/lib/kpi-queries";
import { readFieldValue } from "../src/lib/field-values";
import {
  serializeEntry,
...
import { kpiApiSchema, kpiBaseObjectSchema } from "../src/lib/validation/kpi";
import { fieldDefinitionApiSchema, fieldDefinitionBaseSchema } from "../src/lib/validation/field-definition";
import { buildEntryFieldValuesSchema, entryCoreSchema } from "../src/lib/validation/entry";
import {
  activateKpiRecord,
  archiveKpiRecord,
  createKpiRecord,
  deleteKpiRecord,
  updateKpiRecord,
} from "../src/lib/services/kpi-service";
import {
  addFieldDefinitionRecord,
  deactivateFieldDefinitionRecord,
  deleteFieldDefinitionRecord,
} from "../src/lib/services/field-definition-service";
import {
  createEntryRecord,
  deleteEntryRecord,
  getActiveFieldDefinitions,
  updateEntryRecord,
} from "../src/lib/services/entry-service";

/**
 * MCP server for the KPI dashboard — a third thin entry point over the same
 * lib/services/* functions the Server Actions and the REST API (docs/api.md)
 * use, so business rules stay defined in exactly one place. See docs/mcp.md
 * for how to register this with an MCP client.
 */
...
server.registerTool(
  "list_kpi_fields",
  {
    ...
    description: "All field definitions for a KPI (active and inactive) — the schema a case's `fields` must satisfy.",
    inputSchema: { kpiId: z.string() },
  },
  async ({ kpiId }) => {
    try {
...
server.registerTool(
  "delete_kpi_field",
  {
    ...
    description: "Hard-delete a field. Only allowed if no case has a recorded value for it (409-equivalent otherwise).",
    inputSchema: { kpiId: z.string(), fieldId: z.string() },
  },
  async ({ kpiId, fieldId }) => {
    try {
